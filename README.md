...
The shared eligibility and benchmark rules are in [config/review_analysis.json](config/review_analysis.json). Script 08 and the Python workflow independently implement the same host partition and reference quantile. Run the regression checks with `python -m unittest discover -s tests`. The benchmark is computed from separate reference hosts; the primary RQ contains no fixed numerical count.
