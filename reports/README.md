...
| Table 2 model comparison (the price/minimum-stay variants are in the same table) | `tables/rq_baseline_comparison.csv`, `tables/rq_baseline_fold_comparison.csv` | `11_segment_rate_baseline.py` |
...
| Nested model selection (forest chosen in all five outer folds; AUC +0.059, Brier −0.0055 with host-resampled ranges), probability-bin calibration and within-segment AUC 0.555 | `tables/rq_nested_selection_summary.json`, `rq_nested_selection_folds.csv`, `rq_nested_selection_inner.csv`, `rq_nested_calibration.csv`, `rq_nested_segment_calibration.csv`, `rq_nested_within_segment_auc.csv` | `13_nested_selection.py` |
| Same candidates under support rules of 30, 50 and 75, without the history or price rule, and at review targets of 25, 30, 34 and 35 | `tables/rq_support_rule_sensitivity.csv`, `tables/rq_support_rule_top3.csv`, `tables/rq_threshold_sensitivity.csv` | `12_support_rule_sensitivity.py` |
